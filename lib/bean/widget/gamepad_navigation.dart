...
/// Adds a traversal boundary only on platforms where handheld input is active.
class GamepadFocusScope extends StatelessWidget {
  const GamepadFocusScope({
    super.key,
    required this.node,
    required this.child,
  });
...
class _GamepadNavigationScopeState extends State<GamepadNavigationScope>
  ...
  void initState() {
    super.initState();
    // 非 Linux 不接线：保持透传，避免改变上游在其他平台的焦点/键盘行为。
    if (!isHandheldGamepadSupported()) {
      return;
    }
  ...
  void _dispatch(GamepadCommandEvent event) {
    if (!mounted || !isHandheldGamepadSupported()) {
      return;
    }
  ...
  void _scheduleFocusRecovery() {
    if (!mounted ||
        !isHandheldGamepadSupported() ||
        _focusRecoveryScheduled) {
      return;
    }
    bool usingGamepad;
    try {
      usingGamepad = widget.service.usingGamepad.value;
    } catch (_) {
      return;
    }
    if (!usingGamepad) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (!mounted || !isHandheldGamepadSupported()) return;
      bool stillUsingGamepad;
      try {
        stillUsingGamepad = widget.service.usingGamepad.value;
      } catch (_) {
        return;
      }
      if (!stillUsingGamepad) return;
      final primary = FocusManager.instance.primaryFocus;
      if (primary != null && isGamepadFocusUsable(primary)) return;
  ...
  bool _focusTopRoute(Route<dynamic>? route) {
    ...
    try {
      ...
      final Element? contextElement =
          context is Element ? context as Element : null;
      if (contextElement == null || !contextElement.mounted) {
        return false;
      }
      final focusContext =
  ...
  void _ensurePrimaryFocusVisible() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        ...
        final element = focusContext is Element ? focusContext : null;
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    // 手柄键盘兜底仅在 Linux 生效：其他平台交还给上游默认行为。
    if (!isHandheldGamepadSupported()) {
  ...
  @override
  Widget build(BuildContext context) {
    // 非 Linux 透传：不监听指针、不改 navigationMode，避免影响其他平台。
    if (!isHandheldGamepadSupported()) {
      return widget.child;
    }
