...
class _KeyboardSettingsPageState extends State<KeyboardSettingsPage> {
  ...
  void initState() {
    ...
    if (isHandheldGamepadSupported()) {
      ...
      final storedDeadZone =
          GStorage.getSetting(SettingsKeys.gamepadStickDeadZone);
      gamepadStickDeadZone = storedDeadZone.isFinite
          ? storedDeadZone.clamp(0.15, 0.75).toDouble()
          : 0.5;
      final storedDelay =
          GStorage.getSetting(SettingsKeys.gamepadRepeatDelay);
      gamepadRepeatDelay = storedDelay.clamp(50, 2000).toInt();
    } else {
  ...
  void _configureGamepadService() {
    // 非 Linux 无 scope（AppWidget 透传），maybeServiceOf 为 null 即 no-op。
    if (!isHandheldGamepadSupported()) {
      return;
    }
    if (!mounted) {
    ...
    int repeatMs = gamepadRepeatDelay;
    if (repeatMs < 50) {
      repeatMs = 50;
    } else if (repeatMs > 2000) {
      repeatMs = 2000;
    }
    double deadZone = gamepadStickDeadZone;
    if (!deadZone.isFinite) {
      deadZone = 0.5;
    }
    GamepadNavigationScope.maybeServiceOf(context)?.configure(
      enabled: gamepadEnabled,
      stickDeadZone: deadZone,
      initialRepeatDelay: Duration(milliseconds: repeatMs),
    );
  }
