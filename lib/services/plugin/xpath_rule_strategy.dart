...
class XPathRuleFormatException implements Exception {
  const XPathRuleFormatException(this.message);

  final String message;

  @override
  String toString() => 'XPathRuleFormatException: $message';
}
...
class XPathRuleStrategy {
  ...
  PreparedRuleRequest prepareSearchRequest(
    ...
    if (uri == null || !uri.hasScheme || uri.host.isEmpty) {
      throw XPathRuleFormatException('搜索 URL 无效: $queryUrl');
    }
    if (!config.usePost) {
  ...
  PreparedRuleRequest prepareChapterRequest(
    ...
    if (uri == null || !uri.hasScheme || uri.host.isEmpty) {
      throw XPathRuleFormatException('章节 URL 无效: $url');
    }
  ...
  RuleSearchParseResult parseSearch(
    ...
    for (var index = 0; index < nodes.length; index++) {
      ...
      try {
        final name =
            node.queryXPath(config.searchName).node?.text?.trim() ?? '';
        final source = node
                .queryXPath(config.searchResult)
                .node
                ?.attributes['href']
                ?.trim() ??
            '';
  ...
  RuleChapterParseResult parseChapters(
    ...
    final fragments = <String>[];
    final diagnostics = <String>[];
    final roadNodes = root.queryXPath(config.chapterRoads).nodes;
    for (var roadIndex = 0; roadIndex < roadNodes.length; roadIndex++) {
      ...
      try {
        final urls = <String>[];
        final names = <String>[];
        final episodeNodes = roadNode.queryXPath(config.chapterResult).nodes;
    ...
    if (detectValue.isNotEmpty) {
      switch (config.captchaDetectType) {
        ...
        default:
          ...
          return root.queryXPath(detectValue).node != null;
      }
    }

    final root = htmlElement ?? _documentElement(raw);
    return <String>[config.captchaImage, config.captchaButton]
        .where((xpath) => xpath.trim().isNotEmpty)
        .any((xpath) => root.queryXPath(xpath).node != null);
  }

  Element _documentElement(String raw) {
    final element = parse(raw).documentElement;
    if (element == null) {
      throw const XPathRuleFormatException('HTML 响应没有根节点');
    }
    return element;
  }
